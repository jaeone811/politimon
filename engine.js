...
class GameEngine {
  ...
  effects(source,effects,context={}){
    ...
    for(const e of effects||[]){
      ...
      if(e.type==="selfDamage"&&me.active){me.active.currentHp-=e.amount||0;this.emit(`${me.active.name}에게 반동 ${e.amount||0} 피해`);if(me.active.currentHp<=0)this.removeActive(source,"defeated");}
  ...
  effects(source,effects,context={}){
    ...
    for(const e of effects||[]){
      ...
      if(e.type==="coinFlip"&&typeof window!=="undefined")window.dispatchEvent(new CustomEvent("politimon:coinflip",{detail:{source,...e}}));
      if(e.type==="defeatIfHpAtOrBelow"&&enemy.active&&enemy.active.currentHp<=e.threshold){this.emit(`${enemy.active.name}은(는) 조건 충족으로 즉시 제거됩니다.`);this.removeActive(1-source,"defeated");continue;}
      if(e.type==="gainTokenIfDefeats"){gainTokenOnDefeat+=e.amount||1;continue;}
      if(e.type==="healSelfIfDefeats"){healSelfOnDefeat+=e.amount||0;continue;}
      if(e.type==="healIfDiscarded"){if(context.discardedCharacter&&me.active)me.active.currentHp=Math.min(me.active.hp,me.active.currentHp+(e.amount||0));continue;}
      if(e.type==="requiresStrategyInHand")continue;
      ...
      if(e.type==="turnEnemyActiveFaceDownIfEnemyAttributeIn"){if(enemy.active&&e.attributes?.includes(enemy.active.attribute)){enemy.active.faceDown=true;enemy.traitUsedThisTurn=false;enemy.traitAfterSkillAvailable=false;this.emit(`${enemy.active.name} 카드가 뒷면이 되어 비활성화됩니다.`);}continue;}
      ...
      if(e.type==="stealAllTokensExceptCount"){const keep=e.keep||0,kept=[];for(let i=0;i<keep;i++){const color=settings.tokenTypes.slice().filter(c=>!kept.includes(c)&&(enemy.tokens[c]||0)>0).sort((a,b)=>(enemy.tokens[b]||0)-(enemy.tokens[a]||0))[0];if(color)kept.push(color);}let total=0;settings.tokenTypes.forEach(color=>{let leave=kept.filter(c=>c===color).length,steal=Math.max(0,(enemy.tokens[color]||0)-leave);if(steal){enemy.tokens[color]-=steal;me.tokens[color]=(me.tokens[color]||0)+steal;total+=steal;}});this.emit(`${me.name}이(가) 상대 토큰 ${total}개를 가져왔습니다.`);continue;}
      ...
      if(e.type==="spyTwoEnemyCardsStealOneAndReplace"){const seen=enemy.hand.slice(0,2);if(seen.length){seen.forEach(id=>{const idx=enemy.hand.indexOf(id);if(idx>=0)enemy.hand.splice(idx,1);});const selected=[...seen].sort((a,b)=>rarityRank(a)-rarityRank(b)||a.localeCompare(b))[0],returned=seen.find(id=>id!==selected);if(returned)enemy.hand.push(returned);me.hand.push(selected);const replacement=me.deck.shift();if(replacement)enemy.hand.push(replacement);this.emit(`${me.name}이(가) 상대 손패 2장을 확인하고 1장을 가져왔습니다. 상대는 ${me.name}의 덱에서 1장을 받았습니다.`);}else this.emit(`${enemy.name}의 손패에 확인할 카드가 없습니다.`);continue;}
      ...
      if(e.type==="sendEnemyActiveToDeck"){returnActiveToDeck(1-source);continue;}
      if(e.type==="sendEnemyActiveToDeckIfPackNotName"){if(enemy.active?.type==="character"&&enemy.active?.packIds?.includes(e.packId)&&enemy.active?.name!==e.name)returnActiveToDeck(1-source);else this.emit(`${enemy.active?.name||"상대 인물"}은(는) 효과 대상이 아닙니다.`);continue;}
      ...
      if(e.type==="defeatEnemyAndSelf"){if(enemy.active)this.removeActive(1-source,"defeated");if(me.active)this.removeActive(source,"defeated");continue;}
